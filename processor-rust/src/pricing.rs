...
fn quantity_source_columns(
    ...
    // 先取前一 SKU 右侧、主要 SKU 左侧的最近数量列；主要 SKU 是不可跨越的硬边界。
    let right_quantity = quantity_columns
        .iter()
        .copied()
        .filter(|column| *column > previous_sku && *column < main_sku)
        .min();
    let left_sku_boundary = sku_columns
        ...
        .max();
    let left_quantity = quantity_columns
        .iter()
        .copied()
        .filter(|column| {
            *column < previous_sku && left_sku_boundary.is_none_or(|boundary| *column > boundary)
        })
        .max();
    let quantity = right_quantity
        .or(left_quantity)
...
fn resolve_order_quantities(
    ...
    for indexes in groups.values() {
        let total = indexes.iter().try_fold(0usize, |total, index| {
        ...
        if let Some(total) = total {
            ...
            for index in indexes {
                ...
                resolved[*index].quantity_error = Some("数量合并溢出".to_string());
            }
        }
    }
    resolved
...
fn insert_price_row(
    ...
    // 国家列整格识别（如 UNITED STATES-hold 不会被拆成 US+hold）
    let country = normalize_country_fields(&raw_country, "", "");
...
impl PriceIndex {
    fn lookup_with_single_shipment_preference(
        ...
        if prefer_single_shipment && let Some(single_shipment) = &self.single_shipment {
            let lookup = single_shipment.lookup(country, sku, quantity);
            if lookup.status != "SKU或国家无法匹配" {
                return lookup;
            }
        }
        self.lookup(country, sku, quantity)
    }
        ...
        if let Some(entries) = self.entries.get(&key) {
            if entries.len() != 1 {
                ...
                return Lookup {
                    ...
                    reason: "相同国家、SKU、数量对应多个价格，未静默选择".to_string(),
                };
            }
            let entry = &entries[0];
...
TT TV TW TZ UA UG UM US UY UZ VA VC VE VG VI VN VU WF WS YE YT ZA ZM ZW";
    ...
    #[test]
    fn country_column_matches_whole_cell_without_suffix_split() {
        ...
        assert_eq!(
            index_formal.lookup("US", "ABC123", 1).price,
            Some(8.79),
            "仅正式 UNITED STATES 行应进索引并命中"
        );
    }
